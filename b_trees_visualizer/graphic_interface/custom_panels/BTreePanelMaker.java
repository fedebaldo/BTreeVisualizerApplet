...
import b_trees_visualizer.graphic_interface.listeners.ChildDrawerListener;

public class BTreePanelMaker<E extends Comparable<? super E>> {

	public static BTreeContainerJPanel makePanelFromTree(BTree<?> b, boolean showChanges, boolean keepPrevious, BTreeScrollPanel treePanel) {
		return makePanelFromRoot(b.getRoot(), b.getGrade(), showChanges, keepPrevious, treePanel, (new ArrayList<Integer> ()));
	}

	private static BTreeContainerJPanel makePanelFromRoot(BTreeNode<?> root, int treeGrade, boolean showChanges, boolean keepPrevious, BTreeScrollPanel treePanel, ArrayList<Integer> path) {

			int numberOfElements = (root == null) ? 0 : root.getNumberOfElements();
			...
			if (!root.isLeaf()) {
				...
				for (int i = 0; i < numberOfElements; i++) {
					...
					c.gridx = i;
					c.gridy = 1;
					/* copy of the path from root to the current node*/
					ArrayList<Integer> copy_ = (ArrayList<Integer>) path.clone();
					copy_.add(i);
					BTreeContainerJPanel temp = BTreePanelMaker.makePanelFromRoot(root.getChild(i), treeGrade, showChanges, keepPrevious, treePanel, copy_);
				...
				c.gridy = 1;
				/* copy of the path from root to the current node*/
				ArrayList<Integer> copy__ = (ArrayList<Integer>) path.clone();
				copy__.add(numberOfElements);
				container.add(BTreePanelMaker.makePanelFromRoot(root.getChild(numberOfElements), treeGrade, showChanges, keepPrevious, treePanel, copy__), c);

			}
		} else {
			/*
			* if the node is a leaf for each possible child
			* is created a button
			*/
			JButton emptyLab = new JButton ("");
			emptyLab.addActionListener(new ChildDrawerListener(path, treePanel));
			emptyLab.setOpaque(false);
			c.ipadx = fontSize;
			c.gridx = 0;
			c.gridy = 0;
			dataPanel.add(emptyLab, c);
			c.gridx = 0;
			c.gridy = 0;
			c.ipadx = fontSize;
			c.ipady = fontSize * 2;
